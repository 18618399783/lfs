...
int send_write_req(int sfd)
{
	...
	memset(req_buff,0,sizeof(protocol_header));
	req_header = (protocol_header*)req_buff;
	req_header->header_s.body_len = 0x00;
	req_header->header_s.cmd = PROTOCOL_CMD_WRITE_TRACKER_REQ;

	if((ret = senddata_nblock(sfd,(void*)req_buff,sizeof(req_buff),DEFAULT_NETWORK_TIMEOUT)) != 0)
	{
		printf("send write req to tracker failed.\n");
		return ret; 
	}
	return 0;
}
...
int recv_write_response(int sfd)
{
	...
	char resp_buff[sizeof(lfs_tracker_resp)];

	memset(resp_buff,0,sizeof(lfs_tracker_resp));
	rbytes = 0;
